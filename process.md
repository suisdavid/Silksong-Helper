...
## 进展 16（2026-07-31）：排查"修改未生效"——版本标签流程修正 + 疑似 SilklessCoop 干扰

### 排查过程

用户反馈"并没有修改"。排查时间线：
- v0.9.3 代码 22:37 部署，但**版本字符串在构建后才改**（流程失误）→ DLL 报 0.9.2，无法从日志区分新旧代码。
- 用户 22:40 的日志：如果游戏在 22:37 前就已启动，内存里跑的是旧代码（BepInEx 只在启动时加载插件）。
- 已重新部署（带 0.9.3 版本号），需**完全重启游戏**验证。

### 重大嫌疑：SilklessCoop

日志显示游戏同时加载了 **SilklessCoop 1.0.0（联机合作 Mod）**。"位移后被拉回原点"正是位置同步类 Mod 位置矫正的典型症状——它会认为我们的闪现/位移是"非法移动"并同步回滚。已将 `SilklessCoop.dll` 移至 `BepInEx/plugins_disabled/` 做隔离测试（移回即可恢复）。
